...
function moreExpensive(data){
    let produtoMaisCaro = null
    
    for (let index = 0; index < data.length; index+= 1){
        
        let produtoAtual = data[index]

         if (produtoMaisCaro === null || produtoAtual.price > produtoMaisCaro.price){
                produtoMaisCaro = produtoAtual
            } 
    }
    
    return `O produto mais caro é: ${produtoMaisCaro.name}, que custa: ${produtoMaisCaro.price}`
}

let category = 'sandwiches'

if (mcDonalds[category]) {
    console.log(moreExpensive(mcDonalds[category]));
} else {
